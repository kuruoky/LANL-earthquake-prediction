...
model = RecN()
state_dict = torch.load("model.4.0.pytorch.rnn.pt")
model.load_state_dict(state_dict)

def init_spark():
  spark = SparkSession.builder.appName("output1").config('spark.driver.memory', '4g').getOrCreate()
...
df_f = spark.read.parquet(os.path.join("data", "test.win.vector.fbin.3.parquet"))
...
pd.DataFrame(y_ans).to_csv("result2.csv")
print(y_ans)
