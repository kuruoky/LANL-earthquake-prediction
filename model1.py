...
def init_spark():
  spark = SparkSession.builder.appName("model1").config('spark.driver.memory', '4g').getOrCreate()
...
df_f = spark.read.parquet(os.path.join("data", "train.win.vector.fbin.3.parquet"))
...
for j in range(8):
    ...
    x_temp = torch.Tensor(np.array(x_temp).astype('float32')).reshape(-1, 1, 56)
...
class RecN(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.GRU(56, self.hidden_dim, self.num_layers, batch_first=True, dropout=0.15)
...
for t in range(num_epochs):
    ...
    for i in range(x_train.shape[1]):
        optimiser.zero_grad()
        y_hat, hidden = model(x_train[:, i].view(-1, 1, 56), hidden)
    ...
    for i in range(x_test.shape[1]):
        with torch.no_grad():
            y_hat, hidden = model(x_test[:, i].view(-1, 1, 56), hidden)
...
torch.save(best_model.state_dict(), "model.4.0.pytorch.rnn.pt")
print("done!")
...
for i in range(x_test.shape[1]):
    with torch.no_grad():
        y_hat, hidden = best_model(x_test[:,i].view(-1,1,56), hidden)
